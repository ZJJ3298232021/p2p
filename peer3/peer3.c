...
void add_files_to_tracker(const char *tracker_ip, int tracker_port, const char *peer_ip, int peer_port,
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (is_regular_file(file_path)) {
            ...
            snprintf(torrent_path, sizeof(torrent_path), "peer3/%s.torrent", entry->d_name);
...
void handle_file_transfer(int client_sock) {
    ...
    // 构建文件的完整路径
    char file_path[512];
    snprintf(file_path, sizeof(file_path), "peer3/files/%s", file_name);
...
int main() {
    ...
    int peer_port = 9003;                // 当前 Peer 的端口
    const char *files_directory = "peer3/files"; // 文件目录
